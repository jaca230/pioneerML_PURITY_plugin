...
def _extract_metric_keys(records: Sequence[Mapping[str, Any]]) -> list[str]:
    names: list[str] = []
    for item in records:
    ...
    preferred_order = [
        "trigger_slice_bce",
        "slice_multi_bce",
        "event_builder_bce_proxy",
        "positron_angle_cosine_error",
        "positron_theta_mae_deg",
        "positron_time_mae_proxy_ns",
    ]
...
@PLOT_REGISTRY_DEF.register("purity_task_metric_curves")
class PurityTaskMetricCurvesPlot(BasePlot):
    """Render per-batch task-metric curves (one subplot per metric term)."""

    name = "purity_task_metric_curves"
    ...
    def render(
        ...
        *,
        ...
        title: str = "PURITY Task Metrics (Per Batch)",
        ...
        keys = [str(k) for k in list(metric_keys or []) if str(k).strip() != ""]
        if not keys:
            keys = _extract_metric_keys(records)
        if not keys:
            return None
        ...
        for idx, key in enumerate(keys):
            ...
            y = [float(dict(item.get("metrics") or {}).get(key, float("nan"))) for item in records]
            ...
            ax.plot(x, y, linewidth=1.4, color="tab:green")
